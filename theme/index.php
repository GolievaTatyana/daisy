<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<meta http-equiv="X-UA-Compatible" content="ie=edge">
	<link rel="shortcut icon" href="img/logo.ico" type="image/x-icon">
	<link rel="stylesheet" href="css/normalize.css">
	<link rel="stylesheet" href="css/bootstrap.min.css">
	<link rel="stylesheet" href="css/jquery.fancybox.min.css">
	<link rel="stylesheet" href="css/slick.css">
	<link rel="stylesheet" href="css/slick-theme.css">
	<link rel="stylesheet" href="css/style.css">
	<title>Daisy</title>
</head>
<body>

	<section id="banner">
		<header id="header">
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-10 col-md-offset-1">
						<nav class="navbar" role="navigation">
							<div class="navbar-header">
								<div class="hexagon">
									<button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#bs-example-navbar-collapse-1">
										<span class="sr-only">Toggle navigation</span>
										<span class="icon-bar"></span>
										<span class="icon-bar"></span>
										<span class="icon-bar"></span>
									</button>
								</div>

								<a class="navbar-brand" href="#">
									<img class="img-responsive" src="img/logo.png" alt="logo">
									<span>Daisy</span>
								</a>
							</div>

							<div class="collapse navbar-collapse top-menu" id="bs-example-navbar-collapse-1">
								<ul class="nav navbar-nav">
									<li><a class="active" href="#services">services</a></li>
									<li><a href="#about">about</a></li>
									<li><a href="#portfolio">portfolio</a></li>
									<li><a href="#contact">contact</a></li>
								</ul>
							</div>
						</nav>
					</div>
				</div>
			</div>
		</header>
		<div class="banner-items">
			<div class="slide"></div>
			<div class="slide"></div>
			<div class="slide"></div>
			<div class="slide"></div>
		</div>
		<article id="slogan">
			<div class="container-fluid">
				<div class="row text-center">
					<div class="col-sm-10 col-sm-offset-1 col-lg-8 col-lg-offset-2">
						<div class="slogan">
							<h1><span>Clients</span> in your business</h1>
							<p>Your clients on the internet. Learn how to receive them.</p>
							<a href="#">I want clients</a>
						</div>
					</div>
				</div>
			</div>
		</article>
		<div class="red"><a href="#services"></a></div>
	</section>
	<section id="services">
		<article id="receive">
			<div class="container-fluid">
				<div class="row">
					<div class="col-sm-10 col-sm-offset-1">
						<div class="row text-center caption">
							<div class="col-sm-8 col-sm-offset-2 col-lg-6 col-lg-offset-3">
								<h2>What can you<span> receive?</span></h2>
								<p>Daisy is sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit.</p>
							</div>
						</div>
						<div class="row text-center">
							<div class="col-sm-6 col-md-3">
								<div class="small-hex">
									<div class="hexagon hex-1">
										<div class="hexTop"></div>
										<div class="hexBottom"></div>
									</div>
								</div>
								<h4>clients</h4>
								<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean </p>
							</div>
							<div class="col-sm-6 col-md-3">
								<div class="small-hex">
									<div class="hexagon hex-2">
										<div class="hexTop"></div>
										<div class="hexBottom"></div>
									</div>
								</div>
								<h4>branding</h4>
								<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean </p>
							</div>
							<div class="col-sm-6 col-md-3">
								<div class="small-hex">
									<div class="hexagon hex-3">
										<div class="hexTop"></div>
										<div class="hexBottom"></div>
									</div>
								</div>
								<h4>marketing</h4>
								<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean </p>
							</div>
							<div class="col-sm-6 col-md-3">
								<div class="small-hex">
									<div class="hexagon hex-4">
										<div class="hexTop"></div>
										<div class="hexBottom"></div>
									</div>
								</div>
								<h4>adv.</h4>
								<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean </p>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="row text-center arrow">
				<div class="white"><a href="#business"></a></div>
			</div>
		</article>
		<article id="business">
			<div class="container-fluid">
				<div class="row text-center">
					<div class="border">
						<h1><span>Expand</span> your business</h1>
						<a href="#">Join Us</a>
					</div>
				</div>
			</div>
			<div class="row text-center arrow">
				<div class="red"><a href="#about"></a></div>
			</div>
			<div class="cover"></div>
		</article>
	</section>
	<section id="about">
		<article id="our-process">
			<div class="container-fluid">
				<div class="row">
					<div class="col-sm-10 col-sm-offset-1">
						<div class="row text-center caption">
							<div class="col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2 col-lg-6 col-lg-offset-3">
								<h2>our <span>process</span></h2>
								<p>Daisy is sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. </p>
							</div>
						</div>
						<div class="row text-center">
							<div class="col-md-10 col-md-offset-1">
								<div class="big-hexs hidden-xs">
									<div class="hex-wrapper hex-wrapper-1">
										<div class="hexagon hex-1">
											<div class="icon icon-1"><a href="#"></a></div>
											<div class="hex-content">
												<h4>step<span> 1</span></h4>
												<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida</p>
											</div>
										</div>
									</div>
									<div class="hex-wrapper hex-wrapper-2">
										<div class="hexagon hex-2">
											<div class="hexTop"></div>
											<div class="hexBottom"></div>
										</div>
									</div>
									<div class="hex-wrapper hex-wrapper-3">
										<div class="hexagon hex-3">
											<div class="icon icon-2"><a href="#"></a></div>
											<div class="hex-content">
												<h4>step<span> 2</span></h4>
												<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida</p>
											</div>
										</div>
									</div>
									<div class="hex-wrapper hex-wrapper-4">
										<div class="hexagon hex-4">
											<div class="icon icon-3"><a href="#"></a></div>
											<div class="hex-content">
												<h4>step<span> 3</span></h4>
												<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida</p>
											</div>
										</div>
									</div>
									<div class="hex-wrapper hex-wrapper-5">
										<div class="hexagon hex-5">
											<div class="icon icon-4"><a href="#"></a></div>
											<div class="icon icon-5"><a href="#"></a></div>
											<div class="hex-content">
												<h4>step<span> 4</span></h4>
												<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida</p>
											</div>
										</div>
									</div>
								</div>
								<div class="rounds shov-xs hidden-sm hidden-md hidden-lg">
									<div class="row">
										<div class="col-xs-12">
											<div class="item">
												<div class="round round-1"></div>
												<h4>step<span> 1</span></h4>
												<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida</p>
											</div>
										</div>
										<div class="col-xs-12">
											<div class="item">
												<div class="round round-2"></div>
												<h4>step<span> 2</span></h4>
												<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida</p>
											</div>
										</div>
										<div class="col-xs-12">
											<div class="item">
												<div class="round round-3"></div>
												<h4>step<span> 3</span></h4>
												<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida</p>
											</div>
										</div>
										<div class="col-xs-12">
											<div class="item">
												<div class="round round-4"></div>
												<h4>step<span> 4</span></h4>
												<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida</p>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="row text-center arrow">
				<div class="white"><a href="#we-do"></a></div>
			</div>
		</article>
		<article id="we-do">
			<section id="landing-pg">
				<div class="container-fluid">
					<div class="row">
						<div class="col-sm-10 col-sm-offset-1">
							<div class="wrapper">
								<div class="row text-center caption">
									<div class="col-xs-10 col-xs-offset-1 col-sm-6 col-sm-offset-3">
										<h2>what do <span>we do?</span></h2>
									</div>
								</div>
								<div class="row content">
									<div class="col-md-6 col-md-push-6">
										<div class="devices">
											<img src="img/devices.png" alt="devices" class="img-responsive">
										</div>
									</div>
									<div class="col-md-6 col-md-pull-6 text">
										<h3>Landing Pages</h3>
										<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate
											Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a</p>
											<a href="#">More details</a>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="row text-center arrow">
						<div class="white"><a href="#advertising"></a></div>
					</div>
				</section>
				<section id="advertising">
					<div class="container-fluid">
						<div class="row">
							<div class="col-sm-10 col-sm-offset-1">
								<div class="wrapper">
									<div class="row content">
										<div class="col-md-6">
											<div class="devices">
												<img src="img/device.png" alt="device" class="img-responsive">
											</div>
										</div>
										<div class="col-md-6 text">
											<h3>advertising</h3>
											<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a</p>
											<a href="#">More details</a>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="row text-center arrow">
						<div class="white"><a href="#marketing"></a></div>
					</div>
				</section>
				<section id="marketing">
					<div class="container-fluid">
						<div class="row">
							<div class="col-sm-10 col-sm-offset-1">
								<div class="wrapper">
									<div class="row content">
										<div class="col-md-6 col-md-push-6">
											<div class="devices">
												<img src="img/book.png" alt="book" class="img-responsive">
											</div>
										</div>
										<div class="col-md-6 col-md-pull-6 text">
											<h3>marketing kit</h3>
											<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris. Morbi accumsan ipsum velit. Nam nec tellus a</p>
											<a href="#">More details</a>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="row text-center arrow">
						<div class="white"><a href="#portfolio"></a></div>
					</div>
				</section>
			</article>
		</section>
		<section id="portfolio">
			<div class="container-fluid">
				<div class="row">
					<div class="col-sm-10 col-sm-offset-1">
						<div class="row text-center caption">
							<div class="col-xs-10 col-xs-offset-1 col-sm-6 col-sm-offset-3">
								<h2>Portfolio</h2>
							</div>
						</div>
						<div class="row slider-container">
							<div class="row text-center">
								<ul class="single-item-info">
									<li><a href="#" data-tabindex="0">all</a></li>
									<li><span>/</span></li>
									<li><a href="#" data-tabindex="1">entertainment</a></li>
									<li><span>/</span></li>
									<li><a href="#" data-tabindex="2">sport</a></li>
									<li><span>/</span></li>
									<li><a href="#" data-tabindex="3">business</a></li>
									<li><span>/</span></li>
									<li><a href="#" data-tabindex="4">building</a></li>
								</ul>
							</div>
							<div class="row">
								<div class="single-item">
									<div class="slide">
										<div class="row">
											<div class="col-xs-12 col-sm-6">
												<div class="img-container">
													<div class="img"><img src="img/folio1.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-1">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio2.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-2">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio3.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-3">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-12 col-sm-6 col-sm-push-6">
												<div class="img-container">
													<div class="img"><img src="img/folio6.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-6">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio4.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-4">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio5.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-5">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="row">
											<div class="col-xs-12 col-sm-6">
												<div class="img-container">
													<div class="img"><img src="img/folio1.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-1">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio2.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-2">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio3.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-3">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-12 col-sm-6 col-sm-push-6">
												<div class="img-container">
													<div class="img"><img src="img/folio6.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-4">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio4.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-5">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio5.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-6">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="row">
											<div class="col-xs-12 col-sm-6">
												<div class="img-container">
													<div class="img"><img src="img/folio1.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-1">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio2.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-2">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio3.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-3">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-12 col-sm-6 col-sm-push-6">
												<div class="img-container">
													<div class="img"><img src="img/folio6.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-4">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio4.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-5">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio5.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-6">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="row">
											<div class="col-xs-12 col-sm-6">
												<div class="img-container">
													<div class="img"><img src="img/folio1.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-1">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio2.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-2">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio3.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-3">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-12 col-sm-6 col-sm-push-6">
												<div class="img-container">
													<div class="img"><img src="img/folio6.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-4">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio4.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-5">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio5.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-6">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="row">
											<div class="col-xs-12 col-sm-6">
												<div class="img-container">
													<div class="img"><img src="img/folio1.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-1">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio2.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-2">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3">
												<div class="img-container">
													<div class="img"><img src="img/folio3.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-3">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-12 col-sm-6 col-sm-push-6">
												<div class="img-container">
													<div class="img"><img src="img/folio6.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-4">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio4.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-5">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
											<div class="col-xs-6 col-sm-3 col-sm-pull-6">
												<div class="img-container">
													<div class="img"><img src="img/folio5.png" alt="" class="img-responsive"></div>
													<div class="img-hover">
														<a href="#" class="md-trigger" data-modal="galery-modal-6">
															<p>Item name</p>
															<img src="img/plus.png" alt="plus">
														</a>
													</div>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
						<div class="row text-center oder">
							<a href="#">I want landing page</a>
						</div>
					</div>
				</div>
			</div>
			<div class="row text-center arrow">
				<div class="white"><a href="#team"></a></div>
			</div>
		</section>
		<section id="team">
			<article id="all-members">
				<div class="container-fluid">
					<div class="row">
						<div class="col-sm-10 col-sm-offset-1">
							<div class="row text-center caption">
								<div class="col-xs-10 col-xs-offset-1 col-sm-6 col-sm-offset-3">
									<h2>Our<span> team</span></h2>
								</div>
							</div>
							<div class="row slider-container">
								<div class="all-members">
									<div class="slide text-center">
										<div class="hexagon" style="background-image: url(img/team1.png);">
											<div class="hexTop"></div>
											<div class="hexBottom"></div>
										</div>
										<h3><span>Tomothy</span> Bowman</h3>
										<p>Marketing</p>
										<ul class="socio">
											<li><a href="#"><img src="img/twitter.png" alt="twitter"></a></li>
											<li><a href="#"><img src="img/facebook.png" alt="facebook"></a></li>
											<li><a href="#"><img src="img/in.png" alt="in"></a></li>
										</ul>
									</div>
									<div class="slide text-center">
										<div class="hexagon" style="background-image: url(img/team2.png);">
											<div class="hexTop"></div>
											<div class="hexBottom"></div>
										</div>
										<h3><span>Steve</span> Tracy</h3>
										<p>CEO</p>
										<ul class="socio">
											<li><a href="#"><img src="img/twitter.png" alt="twitter"></a></li>
											<li><a href="#"><img src="img/facebook.png" alt="facebook"></a></li>
											<li><a href="#"><img src="img/in.png" alt="in"></a></li>
										</ul>
									</div>
									<div class="slide text-center">
										<div class="hexagon" style="background-image: url(img/team3.png);">
											<div class="hexTop"></div>
											<div class="hexBottom"></div>
										</div>
										<h3><span>Hellen</span> Johns</h3>
										<p>Clients</p>
										<ul class="socio">
											<li><a href="#"><img src="img/twitter.png" alt="twitter"></a></li>
											<li><a href="#"><img src="img/facebook.png" alt="facebook"></a></li>
											<li><a href="#"><img src="img/in.png" alt="in"></a></li>
										</ul>
									</div>
									<div class="slide text-center">
										<div class="hexagon" style="background-image: url(img/team4.png);">
											<div class="hexTop"></div>
											<div class="hexBottom"></div>
										</div>
										<h3><span>Alan</span> Richman</h3>
										<p>CEO of Romashka</p>
										<ul class="socio">
											<li><a href="#"><img src="img/twitter.png" alt="twitter"></a></li>
											<li><a href="#"><img src="img/facebook.png" alt="facebook"></a></li>
											<li><a href="#"><img src="img/in.png" alt="in"></a></li>
										</ul>
									</div>
									<div class="slide text-center">
										<div class="hexagon" style="background-image: url(img/team1.png);">
											<div class="hexTop"></div>
											<div class="hexBottom"></div>
										</div>
										<h3><span>Tomothy</span> Bowman</h3>
										<p>Marketing</p>
										<ul class="socio">
											<li><a href="#"><img src="img/twitter.png" alt="twitter"></a></li>
											<li><a href="#"><img src="img/facebook.png" alt="facebook"></a></li>
											<li><a href="#"><img src="img/in.png" alt="in"></a></li>
										</ul>
									</div>
									<div class="slide text-center">
										<div class="hexagon" style="background-image: url(img/team2.png);">
											<div class="hexTop"></div>
											<div class="hexBottom"></div>
										</div>
										<h3><span>Steve</span> Tracy</h3>
										<p>CEO</p>
										<ul class="socio">
											<li><a href="#"><img src="img/twitter.png" alt="twitter"></a></li>
											<li><a href="#"><img src="img/facebook.png" alt="facebook"></a></li>
											<li><a href="#"><img src="img/in.png" alt="in"></a></li>
										</ul>
									</div>
									<div class="slide text-center">
										<div class="hexagon" style="background-image: url(img/team3.png);">
											<div class="hexTop"></div>
											<div class="hexBottom"></div>
										</div>
										<h3><span>Hellen</span> Johns</h3>
										<p>Clients</p>
										<ul class="socio">
											<li><a href="#"><img src="img/twitter.png" alt="twitter"></a></li>
											<li><a href="#"><img src="img/facebook.png" alt="facebook"></a></li>
											<li><a href="#"><img src="img/in.png" alt="in"></a></li>
										</ul>
									</div>
									<div class="slide text-center">
										<div class="hexagon" style="background-image: url(img/team4.png);">
											<div class="hexTop"></div>
											<div class="hexBottom"></div>
										</div>
										<h3><span>Alan</span> Richman</h3>
										<p>CEO of Romashka</p>
										<ul class="socio">
											<li><a href="#"><img src="img/twitter.png" alt="twitter"></a></li>
											<li><a href="#"><img src="img/facebook.png" alt="facebook"></a></li>
											<li><a href="#"><img src="img/in.png" alt="in"></a></li>
										</ul>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
				<div class="row text-center arrow">
					<div class="white"><a href="#one-member"></a></div>
				</div>
			</article>
			<article id="one-member">
				<div class="container-fluid">
					<div class="row">
						<div class="col-sm-10 col-sm-offset-1">
							<div class="hex-quotes">
								<div class="hexagon" style="background-image: url(img/quotes.png)">
									<div class="hexTop"></div>
									<div class="hexBottom"></div>
								</div>
							</div>
							<div class="row slider-container">
								<div class="one-member">
									<div class="slide">
										<div class="wrapper">
											<div class="row content">
												<div class="col-sm-12 col-md-4 col-md-push-4">
													<div class="hexagon" style="background-image: url(img/team1.png);">
														<div class="hexTop"></div>
														<div class="hexBottom"></div>
													</div>
												</div>
												<div class="col-sm-12 col-md-4 col-md-push-4 member-caption">
													<h3><span>Tomothy</span> Bowman</h3>
													<p>Marketing</p>
												</div>
												<div class="col-sm-12 col-md-4 col-md-pull-8">
													<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris.</p>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="wrapper">
											<div class="row content">
												<div class="col-sm-12 col-md-4 col-md-push-4">
													<div class="hexagon" style="background-image: url(img/team2.png);">
														<div class="hexTop"></div>
														<div class="hexBottom"></div>
													</div>
												</div>
												<div class="col-sm-12 col-md-4 col-md-push-4 member-caption">
													<h3><span>Steve</span> Tracy</h3>
													<p>CEO</p>
												</div>
												<div class="col-sm-12 col-md-4 col-md-pull-8">
													<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris.</p>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="wrapper">
											<div class="row content">
												<div class="col-sm-12 col-md-4 col-md-push-4">
													<div class="hexagon" style="background-image: url(img/team3.png);">
														<div class="hexTop"></div>
														<div class="hexBottom"></div>
													</div>
												</div>
												<div class="col-sm-12 col-md-4 col-md-push-4 member-caption">
													<h3><span>Hellen</span> Johns</h3>
													<p>Clients</p>
												</div>
												<div class="col-sm-12 col-md-4 col-md-pull-8">
													<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris.</p>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="wrapper">
											<div class="row content">
												<div class="col-sm-12 col-md-4 col-md-push-4">
													<div class="hexagon" style="background-image: url(img/team4.png);">
														<div class="hexTop"></div>
														<div class="hexBottom"></div>
													</div>
												</div>
												<div class="col-sm-12 col-md-4 col-md-push-4 member-caption">
													<h3><span>Alan</span> Richman</h3>
													<p>SEO of Romanskha</p>
												</div>
												<div class="col-sm-12 col-md-4 col-md-pull-8">
													<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris.</p>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="wrapper">
											<div class="row content">
												<div class="col-sm-12 col-md-4 col-md-push-4">
													<div class="hexagon" style="background-image: url(img/team1.png);">
														<div class="hexTop"></div>
														<div class="hexBottom"></div>
													</div>
												</div>
												<div class="col-sm-12 col-md-4 col-md-push-4 member-caption">
													<h3><span>Tomothy</span> Bowman</h3>
													<p>Marketing</p>
												</div>
												<div class="col-sm-12 col-md-4 col-md-pull-8">
													<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris.</p>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="wrapper">
											<div class="row content">
												<div class="col-sm-12 col-md-4 col-md-push-4">
													<div class="hexagon" style="background-image: url(img/team2.png);">
														<div class="hexTop"></div>
														<div class="hexBottom"></div>
													</div>
												</div>
												<div class="col-sm-12 col-md-4 col-md-push-4 member-caption">
													<h3><span>Steve</span> Tracy</h3>
													<p>CEO</p>
												</div>
												<div class="col-sm-12 col-md-4 col-md-pull-8">
													<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris.</p>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="wrapper">
											<div class="row content">
												<div class="col-sm-12 col-md-4 col-md-push-4">
													<div class="hexagon" style="background-image: url(img/team3.png);">
														<div class="hexTop"></div>
														<div class="hexBottom"></div>
													</div>
												</div>
												<div class="col-sm-12 col-md-4 col-md-push-4 member-caption">
													<h3><span>Hellen</span> Johns</h3>
													<p>Clients</p>
												</div>
												<div class="col-sm-12 col-md-4 col-md-pull-8">
													<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris.</p>
												</div>
											</div>
										</div>
									</div>
									<div class="slide">
										<div class="wrapper">
											<div class="row content">
												<div class="col-sm-12 col-md-4 col-md-push-4">
													<div class="hexagon" style="background-image: url(img/team4.png);">
														<div class="hexTop"></div>
														<div class="hexBottom"></div>
													</div>
												</div>
												<div class="col-sm-12 col-md-4 col-md-push-4 member-caption">
													<h3><span>Alan</span> Richman</h3>
													<p>SEO of Romanskha</p>
												</div>
												<div class="col-sm-12 col-md-4 col-md-pull-8">
													<p>This is Photoshop's version  of Lorem Ipsum. Proin gravida nibh vel velit auctor aliquet. Aenean sollicitudin, lorem quis bibendum auctor, nisi elit consequat ipsum, nec sagittis sem nibh id elit. Duis sed odio sit amet nibh vulputate cursus a sit amet mauris.</p>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</article>
		</section>
		<section id="contact">
			<div class="container-fluid">
				<div class="row map">
					<iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d41007.347373759454!2d36.21529570328574!3d50.03086012856666!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0xf93714a3306c2c62!2z0JrQsNC90LDRgtC90LDRjyDQtNC-0YDQvtCz0LA!5e0!3m2!1sru!2sua!4v1502808807767" width="800" height="600" frameborder="0" style="border:0" allowfullscreen></iframe>
				</div>
			</div>
		</section>
		<footer id="footer">
			<div class="row">
				<div class="container-fluid">
					<div class="row hex-adress hidden-xs">
						<div class="hexagon">
							<div class="hex-content">
								<h5>Daisy</h5>
								<p>615 4th Ave S, Jacksonville Beach, FL 32250</p>
							</div>
						</div>
					</div>
					<div class="row hex-adress small hidden-sm show-xs">
						<div class="hexagon">
							<div class="hex-content">
								<h5>Daisy</h5>
								<p>615 4th Ave S, Jacksonville Beach, FL 32250</p>
							</div>
						</div>
					</div>
					<div class="wrapper">
						<div class="row content">
							<div class="logo">
								<a class="navbar-brand" href="#header">
									<img class="img-responsive" src="img/logo.png" alt="logo">
									<span>Daisy</span>
								</a>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="cover"></div>
		</footer>

		<div class="md-modal md-effect-6" id="galery-modal-1">
			<div class="md-content">
				<div class="modal-img">
					<img src="img/folio1_big.png" alt="" class="img-responsive">
				</div>
				<div>
					<button class="md-close"></button>
				</div>
			</div>
		</div>
		<div class="md-modal md-effect-6" id="galery-modal-2">
			<div class="md-content">
				<div class="modal-img">
					<img src="img/folio2_big.png" alt="" class="img-responsive">
				</div>
				<div>
					<button class="md-close"></button>
				</div>
			</div>
		</div>
		<div class="md-modal md-effect-6" id="galery-modal-3">
			<div class="md-content">
				<div class="modal-img">
					<img src="img/folio3_big.png" alt="" class="img-responsive">
				</div>
				<div>
					<button class="md-close"></button>
				</div>
			</div>
		</div>
		<div class="md-modal md-effect-6" id="galery-modal-4">
			<div class="md-content">
				<div class="modal-img">
					<img src="img/folio4_big.png" alt="" class="img-responsive">
				</div>
				<div>
					<button class="md-close"></button>
				</div>
			</div>
		</div>
		<div class="md-modal md-effect-6" id="galery-modal-5">
			<div class="md-content">
				<div class="modal-img">
					<img src="img/folio5_big.png" alt="" class="img-responsive">
				</div>
				<div>
					<button class="md-close"></button>
				</div>
			</div>
		</div>
		<div class="md-modal md-effect-6" id="galery-modal-6">
			<div class="md-content">
				<div class="modal-img">
					<img src="img/folio6_big.png" alt="" class="img-responsive">
				</div>
				<div>
					<button class="md-close"></button>
				</div>
			</div>
		</div>
		<div class="md-overlay"></div>

		<script src="http://ajax.googleapis.com/ajax/libs/jquery/1/jquery.min.js"></script>
		<script src="js/bootstrap.min.js"></script>
		<script src="js/jquery.fancybox.min.js"></script>
		<script src="js/slick.min.js"></script>
		<script src="js/main.js"></script>
		<script src="js/classie.js"></script>
  <!-- <script src="js/css-filters-polyfill.js"></script>
  <script src="js/cssParser.js"></script>
  <script src="js/modernizr.custom.js"></script> -->
  <script src="js/modalEffects.js"></script>
</body>
</html>
